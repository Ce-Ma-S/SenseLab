...
SampleInterfaceWatcher::SampleInterfaceWatcher(AllJoynBusAttachment^ busAttachment) :
...
SampleInterfaceWatcher::~SampleInterfaceWatcher()
{
    UnregisterFromBus();
}

void SampleInterfaceWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.example.SampleInterface" };
...
void SampleInterfaceWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.example.SampleInterface"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.example.SampleInterface"),
            port);
        Added(this, args);
    }
...
void SampleInterfaceWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<SampleInterfaceWatcher>
        };
...
void SampleInterfaceWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_SampleInterfaceIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &SampleInterfaceWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void SampleInterfaceWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void SampleInterfaceWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
